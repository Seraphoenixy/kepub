...
void Epub::flush_font(const std::string &book_name) {
  ...
  } else if (!std::empty(book_name_)) {
    ptr = std::make_unique<klib::ChangeWorkingDir>(book_name_);
  } else {
    klib::error("The book name is empty");
  }

  if (!std::filesystem::exists(Epub::font_path)) {
    klib::warn(
...
void Epub::generate() {
  if (std::empty(book_name_)) {
    klib::error("book name is empty");
  }
  if (std::empty(author_)) {
    klib::error("author is empty");
  }
  if (std::empty(creator_)) {
    klib::error("creator is empty");
  }
